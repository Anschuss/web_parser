...
def get_text(html):
    ...
    demo_title = article.find("article", class_="bg-white shadow").find("span",
                                                                   class_="font-brandUI font-extrabold lg:text-7xl md:text-5xl sm:text-4xl leading-tight").text
    title = normalize_title(demo_title)
    intro = article.find("div",
                         class_="RichText RichText--sans leading-loose lg:text-xl md:text-xl sm:text-l lg:mb-32 md:mb-32 sm:mb-24").text
